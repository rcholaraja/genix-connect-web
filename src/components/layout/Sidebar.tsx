...
import {
  LayoutDashboard, Users, CalendarCheck, Wallet, GraduationCap,
  MessageSquare, Calendar, FileText, BarChart2, LogOut, Megaphone, Menu, X,
} from 'lucide-react';
...
const NAV_ITEMS = [
  ...
  { label: 'Exams & Scores', href: '/exams', icon: GraduationCap },
  { label: 'Messages', href: '/messages', icon: MessageSquare },
  { label: 'Announcements', href: '/announcements', icon: Megaphone },
  { label: 'Calendar', href: '/calendar', icon: Calendar },
  { label: 'Leave Requests', href: '/leave-requests', icon: FileText },
  { label: 'Attendance Report', href: '/report', icon: BarChart2 },
];
    ...
    <aside className="w-64 h-full flex flex-col"
      ...
      <div className="flex items-center gap-3 px-5 py-4 border-b border-white/10">
        ...
        <div className="min-w-0">
          <p className="font-semibold text-white text-sm truncate">{userProfile?.name ?? 'Teacher'}</p>
          <span className="text-[10px] bg-white/20 text-white font-medium px-2 py-0.5 rounded-full">Teacher</span>
        </div>
      </div>
