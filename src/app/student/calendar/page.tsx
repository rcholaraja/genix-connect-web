...
import { ChevronLeft, ChevronRight, PartyPopper, FlaskConical, BookOpen, Star, CalendarDays, Cake } from 'lucide-react';
...
export default function StudentCalendarPage() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      getStudents(),
    ]).then(([evts, studs]) => { setEvents(evts); setStudents(studs); })
      .catch(console.error)
      .finally(() => setLoading(false));
  }, [month]);

  // Birthday virtual events for this month
  const birthdaysByDate: Record<string, { name: string; age: number }[]> = {};
  students.forEach((s: any) => {
    if (!s.dob) return;
    const mmdd = s.dob.slice(5);
    const dateStr = `${month.format('YYYY')}-${mmdd}`;
    const d = dayjs(dateStr);
    if (!d.isValid() || d.month() !== month.month()) return;
    if (!birthdaysByDate[dateStr]) birthdaysByDate[dateStr] = [];
    birthdaysByDate[dateStr].push({ name: s.name, age: month.year() - dayjs(s.dob).year() });
  });
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 items-start">
        <div className="col-span-1 md:col-span-2 bg-white rounded-xl shadow-sm border border-gray-100 p-5">
          ...
          <div className="flex gap-4 mt-4 justify-center flex-wrap">
            {['holiday', 'test', 'class', 'special', 'birthday'].map(t => {
